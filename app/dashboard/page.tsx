...
export default async function DashbboardPage(){
    ...
    prisma.product.findMany({
        ...
        const lowStockCount = allProducts.filter((p) =>(p.quantity)<=5 && Number(p.quantity) >= 1).length;
         const outofStockCount = allProducts.filter((p) =>(p.quantity)===0).length;
         const inStockPercentage = totalProduct > 0?(inStockCount/totalProduct)*100:0; 
        const lowstockStockPercentage = totalProduct > 0?(lowStockCount/totalProduct)*100:0; 
        const outStockPercentage = totalProduct > 0?(outofStockCount/totalProduct)*100:0; 
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-semibold text-gray-900">
                        ...
                        Welcome back ! here is an overview of your Inventory</p>
                </div>
            </div>
         </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
                ...
                <div className="grid grid-cols-3 gap-6">
                 ...
                 <div className="text-3xl font-bold text-gray-900">
                    ₹{Number(totalValue).toFixed(0)}</div>
                 ...
                 <div className="flex items-center justify-center mt-1">
                   ...
                   <div className="bg-white rounded-lg border border-gray-200 p-6">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="h-48"><ProductsChart data = {weeklyProductsData}/>
                        </div>
                            </div>
                            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="mt-6 space-y-2">
              <div className="flex items-center justify-between text-sm text-gray-600">
                <div className="flex items-center space-x-2">
                  ...
                  <span>In Stock ({inStockPercentage.toFixed(2)}%)</span>
                </div>
              </div>
              <div className="flex items-center justify-between text-sm text-gray-600">
                <div className="flex items-center space-x-2">
                  <div className="w-3 h-3 rounded-full bg-purple-600" />
                  <span>Low Stock ({lowstockStockPercentage.toFixed(2)}%)</span>
                </div>
              </div>
              <div className="flex items-center justify-between text-sm text-gray-600">
                <div className="flex items-center space-x-2">
                  <div className="w-3 h-3 rounded-full bg-gray-200" />
                  <span>Out of Stock ({outStockPercentage.toFixed(2)}%)</span>
                </div>
              </div>
            </div>
          </div>
